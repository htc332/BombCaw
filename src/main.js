...
class BombWallGame {
  constructor() {
    ...
    this.debugLogs = [];
    this.maxLogs = 20;
    this.isDebugMode = false; // [v0.9.0-fix] 关闭调试模式
  ...
  initSystems() {
    ...
    // [v0.9.1] 初始化通关弹窗
    this.levelCompletePopup = new LevelCompletePopup(this.canvas);
    this.levelCompletePopup.onNextLevel = () => {
      this.hideLevelComplete();
      var nextLevel = this.levelSystem.completeLevel(this.gameLogic.level, this.gameLogic.score);
      this.startLevel(nextLevel);
    };
    
    // [v0.9.2] 初始化失败弹窗
    this.levelFailPopup = new LevelFailPopup(this.canvas);
    this.levelFailPopup.onWatchAd = () => {
      this.handleWatchAdForScore();
    };
    this.levelFailPopup.onWaitRestart = () => {
      this.hideLevelFail();
      // [v0.9.9-fix] 失败回到第1关时重置积分为初始值
      this.gameLogic.score = 10;
      this.startLevel(1); // 回到第1关
    };
  ...
  handleTouch(x, y) {
    ...
    // [v0.9.1] 优先检测通关弹窗点击
    if (this.gameState === 'victory' && this.levelCompletePopup) {
      if (this.levelCompletePopup.onTouch(x, y)) {
        return;
      }
    }
    
    // [v0.9.2] 优先检测失败弹窗点击
    if (this.gameState === 'failed' && this.levelFailPopup) {
      if (this.levelFailPopup.onTouch(x, y)) {
        return;
      }
    }
  ...
  onLevelComplete(event) {
    ...
    // [v0.9.0] 使用DDA计算下一关
    var nextLevel = this.levelSystem.completeLevel(event.level, event.score);
    
    // [v0.9.1] 显示通关弹窗
    this.levelCompletePopup.show({
      level: event.level,
      score: event.score,
      bombsPlaced: event.bombsPlaced || 0,
      wallsDestroyed: event.wallsDestroyed || 0,
      nextLevel: nextLevel
    });
  ...
  onLevelFailed(event) {
    ...
    // [v0.9.2] 新计分系统：失败不再重置分数，保留当前分数用于广告复活
    // 计算本局失败次数（用于广告奖励递增）
    var failCount = (this.storage.get('level_' + this.gameLogic.level + '_fails') || 0) + 1;
    this.storage.set('level_' + this.gameLogic.level + '_fails', failCount);
    
    // 广告奖励：第1次10分，第2次20分，第3次50分
    var adReward = 10;
    if (failCount === 2) adReward = 20;
    else if (failCount >= 3) adReward = 50;
  ...
  loop() {
    try {
      ...
      // [v0.9.1] 更新通关弹窗动画
      if (this.gameState === 'victory' && this.levelCompletePopup) {
        this.levelCompletePopup.update(dt);
      }
      
      // [v0.9.2] 更新失败弹窗动画
      if (this.gameState === 'failed' && this.levelFailPopup) {
        this.levelFailPopup.update(dt);
      }
  ...
  render(dt) {
    try {
      ...
      ctx.restore(); // 恢复 shake 的 save
      
      // [v0.9.1] 绘制通关弹窗（在最上层，不使用 Canvas 变换）
      if (this.gameState === 'victory' && this.levelCompletePopup) {
  ...
  renderUI() {
    ...
    if (this.gameState === 'victory') {
      // [v0.9.1] 新通关弹窗由 LevelCompletePopup 绘制，renderUI 只绘制半透明背景
      ctx.fillStyle = 'rgba(0, 0, 0, 0.75)';
      ctx.fillRect(0, 0, w, h);
    }
    
    if (this.gameState === 'failed') {
      // [v0.9.2] 新失败弹窗由 LevelFailPopup 绘制，renderUI 只绘制半透明背景
      ctx.fillStyle = 'rgba(0, 0, 0, 0.75)';
      ctx.fillRect(0, 0, w, h);
    }
  }
  ...
  showLevelComplete(data) {
    this.levelCompletePopup.show(data);
  }

  showLevelFail(data) {
    this.levelFailPopup.show(data);
  }

  handleWatchAdForScore() {
    // [v0.9.2] 处理观看广告获得积分
    var self = this;
    this.adManager.showRewardedAd({
      onSuccess: function(res) {
        // 获取当前失败次数对应的奖励
        var failCount = self.storage.get('level_' + self.gameLogic.level + '_fails') || 1;
        var reward = 10;
        if (failCount === 2) reward = 20;
        else if (failCount >= 3) reward = 50;
        
        self.gameLogic.score += reward;
        self.hideLevelFail();
        self.gameState = 'playing';
        self.showHint('获得 ' + reward + ' 积分!');
      },
      onFail: function(res) {
        self.showHint('广告未看完，未获得积分');
      }
    });
  }
}
